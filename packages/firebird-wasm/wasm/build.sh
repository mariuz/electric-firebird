...
# Validate every patch file first, whether or not they have been applied.
#
# A patch with more than one "diff --git" line for the same file is malformed,
# and git rejects the whole thing as "patch with only garbage". It happens when
# a patch is regenerated by concatenating a prose header onto a fresh
# `git diff` with the header cut a line or two late, taking the start of the
# old diff with it. That has now happened twice.
#
# Deliberately outside the marker check below: a tree that already has the
# patches applied skips them entirely, so a malformed file stays invisible
# locally and fails only on a clean checkout — which is to say, only in CI.
for p in "${PATCHES_DIR}"/00*.patch; do
  [[ -f "$p" ]] || continue
  headers=$(grep -c "^diff --git" "$p" || true)
  distinct=$(grep "^diff --git" "$p" | sort -u | wc -l)
  if (( headers > distinct )); then
    echo "Error: $(basename "$p") repeats a 'diff --git' header ($headers lines, $distinct distinct)." >&2
    echo "       git will reject it as \"patch with only garbage\"." >&2
    echo "       Regenerate with the prose header cut at the FIRST 'diff --git'," >&2
    echo "       followed by the output of git diff." >&2
    exit 1
  fi
done
